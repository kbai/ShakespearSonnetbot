...
class modelhmm():
    ...
    def savemodel(self):
        np.savetxt('../reversemodel/'+self.filename+'trans.txt',self.trans_)
        np.savetxt('../reversemodel/'+self.filename+'obs.txt',self.obs_)


    def loadmodel(self):
        self.trans_ = np.loadtxt('../reversemodel/'+self.filename+'trans.txt')
        self.obs_ = np.loadtxt('../reversemodel/'+self.filename+'obs.txt')


    def analyzing_word(self,words):
        df = pd.DataFrame((self.obs_).transpose(),index=words)
        df.to_csv('../reversemodel/'+self.filename+'withword.txt',index=True,header=True,sep=' ')
...
def poem_generate(num_of_hidden_states, num_pairs):
    ...
    for ind in ['A','B','C','D','E','F','G']:
        ...
        for i in range(ntrial):
            ...
            hmm = modelhmm(num_of_hidden_states, len(words), Y, modelname)
    ...
    poem_file_name = '../poems/reverse_'+str(num_of_hidden_states)+'.txt'
